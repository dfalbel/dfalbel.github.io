...
* Não fica claro quais são os valores dos hiperparâmetros que serão utilizados.
...
classif_lda <- R6Class("classif_lda",
  public = list(
    ...
    predict = function(X) {
      p <- predict(self$fitted_model, newdata = X, method = self$predict.method)
      p$posterior
    }
  )
)
...
train <- function(X, Y, model) {
  m <- model$clone()
  m$fitted_model <- m$fit(X, Y)
  return(m)
}
```

A função `train` poderia receber instruções mais complexas para fazer cross-validation,
calcular algumas medidas de acerto, fazer alguns pre-processamentos na base. 
Além disso, `train` poderia ter interfaces para fórmulas e especificação dos dados.

```{r}
model <- classif_lda$new()
fit <- train(X, Y, model)
```

Também é possível adicionar uma função geral de *scoragem* do modelo por exemplo.
...
Essa função `score` em um contexto mais geral, pode definir diversas formas de escoragem,
como por exemplo: retornar apenas a classe ou uma matriz com as probabilidades.
