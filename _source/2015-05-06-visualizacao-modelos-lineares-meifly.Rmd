...
Nos últimos dias, dei um fork no repositório do [meifly no Github]() e fiz algumas alterações no código, trocando alguns códigos pelas funções do pacote [broom](). Neste post, vou usar o pacote com as alterações que fiz, então para instalá-lo basta usar `devtools::install_github("dfalbel/meifly")`.
...
Estammos interessados em encontrar um bom modelo para prever o preço do diamante de acordo com as demais características que estão na base de dados.
...
As visualizações, aqui, ficam um pouco mais complicadas uma vez que os parâmetros estão em escalaas diferentes e são difíceis de serem comparados. No entanto, o gráfico a seguir pode dizer alguma coisa:
